...
public class SoftlandIntegration
    {
        ...
        public int ProcesaCabecera(XmlDocument doc, string cadenaConexion, int tipoDTE, DateTime fechEmis, int folioFactura, string rutEmisor, string rutaError, string nameFile, string spCabecera)
        {
            int newProdID = 0;
            try
            {
                    ...
                    foreach (XmlNode xmlNode in listaNodosDTE)
                    {
                        using (SqlCommand command = new SqlCommand(spCabecera, connection))
                        {
                            ...
                            if (xmlNode["Totales"] != null)
                            {
                                ...
                                {
                                    command.Parameters.AddWithValue("@Observaciones", SqlDbType.VarChar).Value = xmlNode["Totales"].GetElementsByTagName("Observaciones")[0].InnerText;
                                }
                                else
                                {
                                    command.Parameters.AddWithValue("@Observaciones", SqlDbType.VarChar).Value = DBNull.Value;

                                }
                                if (xmlNode["Totales"].GetElementsByTagName("NombreObra").Count != 0)
                                {
                                    command.Parameters.AddWithValue("@NombreObra", SqlDbType.VarChar).Value = xmlNode["Totales"].GetElementsByTagName("NombreObra")[0].InnerText;
                                }
                                else
                                {
                                    command.Parameters.AddWithValue("@NombreObra", SqlDbType.VarChar).Value = DBNull.Value;


                                }
                                if (xmlNode["Totales"].GetElementsByTagName("DirObra").Count != 0)
                                {
                                    command.Parameters.AddWithValue("@DirObra", SqlDbType.VarChar).Value = xmlNode["Totales"].GetElementsByTagName("DirObra")[0].InnerText;
                                }
                                else
                                {
                                    command.Parameters.AddWithValue("@DirObra", SqlDbType.VarChar).Value = DBNull.Value;
                            ...
                            */
                            ...
                            {
                                command.Parameters.AddWithValue("@FolioReferencia", SqlDbType.VarChar).Value = folioReferencia;
                            }
                            else
                            {
                                command.Parameters.AddWithValue("@FolioReferencia", SqlDbType.VarChar).Value = DBNull.Value;
                            }
